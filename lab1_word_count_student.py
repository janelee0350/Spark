...
len(list(wordsGrouped.collect()[0])[1])
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda x : (list(x)[0], len(list(x)[1])))
...
def wordCount(wordListRDD):
    ...
    """
    wordcount = wordListRDD.map(lambda x : (x ,1)).reduceByKey(lambda a,b : a+b)
    return wordcount
...
def removePunctuation(text):
    ...
    text_new = re.sub(pattern, ' ', text_new)
