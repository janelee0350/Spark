...
def parsePoint(line):
    ...
    point = line.encode("utf-8").split(',')
    pointf = map(eval, point)
    # 或 pointf = [float(i) for i in point]
    # 或下面三种方法
    labeledPoint = LabeledPoint(pointf[0], pointf[1:])
    return labeledPoint
...
np.array(parsedSampleMorePoints[1].features)
...
parsedSampleMorePoints = map(parsePoint, sampleMorePoints)
dataValues = map(lambda lp: np.array(lp.features), parsedSampleMorePoints)
...
parsedDataInit.take(2)


# In[338]:

parsedDataInit.take(2)[1].label
...
print type(parsedDataInit.take(1)[0])


# In[343]:

# TODO: Replace <FILL IN> with appropriate code
def shiftYear(parsedDataInit):
    parsedDataInit.label = parsedDataInit.label - 1922.0
    return parsedDataInit

parsedData = parsedDataInit.map(lambda x : shiftYear(x))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    error = labelsAndPreds.map(lambda (x,y):(x-y)**2).reduce(lambda a,b:a+b)
    mse = error/labelsAndPreds.count()
    rmse = mse**0.5
    return rmse
...
print averageValYear


# In[379]:

# TODO: Replace <FILL IN> with appropriate code
averageTestYear = (parsedTestData
                    .map(lambda x : x.label)
                    .mean()
                   )
                   
                
print averageTestYear
...
def gradientSummand(weights, lp):
    ...
    y = lp.label
    x = lp.features
    return (weights.dot(np.array(x)) - np.array(y)) * np.array(x)    
    # a.dot(b) : a 点乘 b
...
features_inter = itertools.product(lp.features, lp.features)
lf = list(features_inter)
...
features_add = []
for feature in lf:
    features_add.append(feature[0]*feature[1])


# In[506]:

features_add.extend(lp.features.tolist())
...
def twoWayInteractions(lp):
    ...
    features_inter = list(itertools.product(lp.features, lp.features))
    features_add = []
    
    for feature in features_inter :
        features_add.append(feature[0]*feature[1])
    
    features_interaction = []
    features_interaction.extend(lp.features)
    features_interaction.extend(features_add)
    lp_inter = LabeledPoint(lp.label, list(features_interaction))
    return lp_inter
...
len(parsedTrainData.map(lambda lp:twoWayInteractions(lp)).take(1)[0].features)


# In[523]:

len(parsedTrainData.take(1)[0].features)
...
InterTrainData = parsedTrainData.map(lambda lp:twoWayInteractions(lp))
InterValData = parsedValData.map(lambda lp:twoWayInteractions(lp))
modelInteract = LinearRegressionWithSGD.train(InterTrainData, numIters, alpha,
                                              miniBatchFrac, regParam=reg,
                                              regType='l2', intercept=True)
labelsAndPredsInteract = InterValData.map(lambda lp: (lp.label, modelInteract.predict(lp.features)))
...
# TODO: Replace <FILL IN> with appropriate code
InterTestData = parsedTestData.map(lambda lp:twoWayInteractions(lp))
labelsAndPredsTest = InterTestData.map(lambda lp: (lp.label, modelInteract.predict(lp.features)))
